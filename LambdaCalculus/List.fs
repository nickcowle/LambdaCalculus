...
module List =
    ...
    let foldT = Functions.Y $ lams 4 (Var 0 $ Var 1 $ lams 2 (Var 4 $ appVars [ 5 ; 4 ; 3 ; 0 ] $ Var 1))

    /// lengthT : 'a list -> Nat
    let lengthT = foldT $ lams 2 (Nat.succT $ Var 1) $ Nat.zeroT

    /// appendT : 'a list -> 'a list -> 'a list
    let appendT = Functions.Y $ lams 3 (Var 1 $ Var 0 $ lams 2 (consT $ Var 1 $ appVars [ 4 ; 0 ; 2 ]))
